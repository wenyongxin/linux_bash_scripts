...
ZABBIX_DIR=`pwd`
date_site="baidu.com:8080"
system=$1
public=$2

#zabbix proxy的ip地址
proxy_info(){
	echo -e "\033[49;32;1m 中国香港		8.8.8.8 \033[0m"
	echo -e "\033[49;32;1m 腾 讯 云		8.8.8.8 \033[0m"
        echo -e "\033[49;32;1m 台湾远传		8.8.8.8 \033[0m"
        echo -e "\033[49;32;1m 台湾中华		8.8.8.8 \033[0m"
        echo -e "\033[49;32;1m 韩    国		8.8.8.8 \033[0m"
        echo -e "\033[49;32;1m 东 南 亚		8.8.8.8 \033[0m"
        echo -e "\033[49;32;1m 欧    洲		8.8.8.8 \033[0m"
        echo -e "\033[49;32;1m 美    洲		8.8.8.8 \033[0m"
        echo -e "\033[49;32;1m 悉    尼		8.8.8.8 \033[0m"
        echo -e "\033[49;32;1m 日    本		8.8.8.8 \033[0m"
}
...
INSTALL_PT(){
	wget http://$date_site/pt/key-pt.tar > /dev/null 2>&1 && echo -e "\033[49;32;1m $public \033[0m key-pt.tar is down"
	[ $? -eq 0 ] && wget http://$date_site/pt/log_scripts.tar.gz  > /dev/null 2>&1 && echo -e "\033[49;32;1m $public \033[0m key-pt.tar is down"
...
DOWN(){
	wget http://$date_site/client/key.tar > /dev/null 2>&1 && echo -e "\033[49;32;1m $public \033[0m key.tar is ok"
	[ $? -eq 0 ] && wget http://$date_site/client/net-snmp-5.7.2.tar.gz > /dev/null 2>&1 && echo -e "\033[49;32;1m $public \033[0m net-snmp-5.7.2.tar.gz is ok"
	[ $? -eq 0 ] && wget http://$date_site/client/port.tar > /dev/null 2>&1 && echo -e "\033[49;32;1m $public \033[0m port.tar is ok"
	[ $? -eq 0 ] && wget http://$date_site/client/zabbix-2.4.4.tar.gz > /dev/null 2>&1 && echo -e "\033[49;32;1m $public \033[0m zabbix-2.4.4.tar.gz is ok"
	[ $? -eq 0 ] && wget http://$date_site/client/zabbix_agentd.conf > /dev/null 2>&1 && echo -e "\033[49;32;1m $public \033[0m zabbix_agentd.conf is ok"
}
...
iptables_cmd(){
	...
	if [ ! -f "/tmp/iptable" ]; then
		if [ -f "/etc/sysconfig/iptables" ];then
			...
			if [ $? -eq 0 ];then
				wget -P $ZABBIX_DIR http://$date_site/client/iptables_comd.txt > /dev/null 2>&1
				...
				wget -P $ZABBIX_DIR http://$date_site/client/iptables_conf.txt > /dev/null 2>&1
				while read iptable;do
...
#centos/redhat
	c )
		...
		[ $? -eq 0 ] && wget http://$date_site/client/snmpd.conf.yum > /dev/null 2>&1 && echo -e "\033[49;32;1m $public \033[0m snmpd.conf is ok"
...
#ubuntu
	u )
		...
		[ $? -eq 0 ] && wget http://$date_site/client/snmpd.conf > /dev/null 2>&1 && echo -e "\033[49;32;1m $public \033[0m Down snmpd.conf ok"
...
EOF
		useradd -M -s /sbin/nologin zabbix
		echo "=========== $public Download File ==========" 
		wget http://$date_site/client/snmpd.conf > /dev/null 2>&1 && echo -e "\033[49;32;1m $public \033[0m Down snmpd.conf ok"
		DOWN
		echo "=========== $public Install SNMP ============" 
		SNMP
		echo "=========== $public Install Agent ==========" 
...
#freebsd
	f )
		...
		pkg_add http://$date_site/client/pack_freebsd/perl-5.14.4.tbz > /dev/null 2>&1 && echo "$public perl is install"
		[ $? -eq 0 ] && pkg_add http://$date_site/client/pack_freebsd/net-snmp.tbz > /dev/null 2>&1 && echo "$public net-snmp is install"
		echo "================= $public Install End ======================" 
		pw user add zabbix -s /sbin/nologin
		echo "=========== $public Download File ==========" 
                DOWN
		echo "=========== $public Install SNMP ============" 
		wget http://$date_site/client/snmpd.conf > /dev/null 2>&1 && echo "$public Down snmpd.conf ok"
